/// RIPEMD-160 hash function — standalone pure-Zig implementation.
/// Extracted from src/evm/precompile/ripemd160.zig for use as a crypto backend.
///
...
const Context = struct {
    ...
    pub fn init() Context {
        return Context{
            .h = [_]u32{ 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0 },
            .total = 0,
            .buf = [_]u8{0} ** 64,
            .buflen = 0,
        };
    ...
    pub fn final(self: *Context) [20]u8 {
        ...
        for (0..5) |i| std.mem.writeInt(u32, output[i * 4 ..][0..4], self.h[i], .little);
        return output;
    }
